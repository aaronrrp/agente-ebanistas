...
async function runDesignBreakdownFollowup(imageData, message) {
  ...
  const dots = document.createElement("span"); dots.className = "loading-dots"; dots.innerHTML = "<span></span><span></span><span></span>"; pending.appendChild(dots);
  els.chatMessages.scrollTop = els.chatMessages.scrollHeight;
  const promptMsg = (message ? message + ". " : "") + "Este es el boceto/render de un mueble. PRIMERO revisa si en la imagen o en el texto hay MEDIDAS EXTERIORES claras (ancho, alto, profundidad del mueble). Si NO las hay (un boceto a mano casi nunca las trae, y el área del cuarto NO cuenta como medida del mueble): responde CORTO y rápido — di qué tipo de mueble ves e indícame en UNA frase que me hacen falta las 3 medidas exteriores (ancho x alto x profundidad) para hacer el despiece exacto. NO inventes medidas ni generes el despiece sin ellas. Si SÍ hay medidas exteriores claras: dame el DESPIECE COMPLETO en melamina 18mm — cada pieza con nombre, medidas en mm, cantidad y canteado, incluyendo el array 'pieces' para Cortes, con defaults de fabricación (fondo embutido, puertas sobrepuestas, correderas telescópicas).";
  try {
