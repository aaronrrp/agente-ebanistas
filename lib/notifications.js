...
async function notify(userRef = {}, { title, body = "", kind = "info", link = "", email, sendPush = false } = {}) {
  ...
  if (to) {
    const html = `<div style="font-family:Arial,sans-serif;max-width:520px;margin:0 auto;color:#1a1346">
      <h2 style="color:#3B2D8F">${esc(title)}</h2>
      <p>${esc(body)}</p>
      ${link ? `<p><a href="${esc(link)}" style="color:#3B2D8F">Ver más</a></p>` : ""}
