...
function reviewPendingHtml(name, kind) {
  ...
  return `<div style="font-family:Arial,sans-serif;max-width:520px;margin:0 auto;color:#2D235F">
    <h2 style="color:#2D235F">¡Recibimos tu solicitud!</h2>
...
function approvedHtml(name, kind, accessCode) {
  const rol = kind === "company" ? "empresa" : "perfil profesional";
  return `<div style="font-family:Arial,sans-serif;max-width:520px;margin:0 auto;color:#2D235F">
    <h2 style="color:#2D235F">¡Tu ${rol} fue aprobado! 🎉</h2>
    <p>Hola ${escapeText(name)},</p>
