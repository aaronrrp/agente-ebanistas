...
// Auditoría v51: sin Access-Control-Allow-Origin — la app es same-origin y los
// tokens viajan por header Authorization; ningún flujo legítimo necesita CORS.
...
function readBody(req) {
  return new Promise((resolve, reject) => {
    ...
    req.on("data", c => {
      body += c;
      // Al superar el límite se destruye el stream — sin esto el servidor seguía
      // acumulando el resto del body en memoria aunque la promesa ya había fallado.
      if (body.length > 10_000_000) { reject(new Error("Too large")); req.destroy(); }
    });
...
// ── Barrido periódico de sesiones ─────────────────────────────────────────────
// Cada Map de sesiones borra entradas caducadas solo al consultarlas (lazy): un
// token que nunca vuelve a usarse quedaba en memoria para siempre. Con miles de
// logins eso es un leak. Cada módulo registra aquí su Map y cada 10 minutos se
// eliminan las entradas vencidas. Acepta ambos formatos de valor:
// timestamp plano (adminSessions) u objeto { ..., ts } (el resto).
const _sweepTargets = [];
function registerSessionSweep(map, ttlMs) {
  _sweepTargets.push({ map, ttlMs });
}
setInterval(() => {
  const now = Date.now();
  for (const { map, ttlMs } of _sweepTargets) {
    for (const [k, v] of map) {
      const ts = typeof v === "number" ? v : v?.ts;
      if (now - (ts || 0) > ttlMs) map.delete(k);
    }
  }
}, 10 * 60 * 1000).unref();
registerSessionSweep(adminSessions, SESSION_TTL);

// ── Registro de sesiones multi-rol ────────────────────────────────────────────
// Cada módulo con su propio Map de sesiones (ebanista/vendedor en server.js,
// profesional/empresa/usuario gratuito en routes/) registra aquí su verificador.
// Permite a endpoints compartidos (p.ej. subida de imágenes) exigir "cualquier
// sesión válida" sin imports circulares entre módulos.
const _sessionCheckers = [];
function registerSessionChecker(fn) { _sessionCheckers.push(fn); }
function isAnyValidSession(token) {
  if (!token) return false;
  if (isValidSession(token)) return true; // admin
  return _sessionCheckers.some(fn => { try { return Boolean(fn(token)); } catch { return false; } });
}
...
module.exports = {
  ...
  registerSessionChecker, isAnyValidSession, registerSessionSweep
};
